...
from utils import (losses, ramps, feature_memory, contrastive_losses,
                   val_2d_mri_interp as val_2d)


class MRILinearRandomGenerator(object):
    """PROMISE12 augmentation with continuous interpolation for MRI only.

    MRI intensities are continuous, so rotations and resizing use bilinear
    interpolation (SciPy order=1).  Discrete segmentation labels deliberately
    retain nearest-neighbour interpolation (order=0).
    """
    ...
    def __call__(self, sample):
        image, label_map = sample['image'], sample['label']
        if random.random() > 0.5:
            ...
            label_map = np.flip(label_map, axis=axis).copy()
        elif random.random() > 0.5:
            angle = np.random.randint(-20, 20)
            image = ndimage.rotate(
                image, angle, order=1, reshape=False, prefilter=False)
            label_map = ndimage.rotate(
                label_map, angle, order=0, reshape=False, prefilter=False)

        height, width = image.shape
        scale = (self.output_size[0] / height,
                 self.output_size[1] / width)
        image = zoom(image, scale, order=1, prefilter=False)
        label_map = zoom(label_map, scale, order=0, prefilter=False)
        return {
            'image': torch.from_numpy(
                image.astype(np.float32)).unsqueeze(0),
            'label': torch.from_numpy(label_map.astype(np.uint8)),
        }
...
parser.add_argument('--exp', type=str,
                    default='MT_PROMISE12_UniMatch_MRIInterp',
...
def pre_train(args, snapshot_path):
    ...
    db_train = BaseDataSets(base_dir=args.root_path, split="train", num=None, transform=transforms.Compose([
        MRILinearRandomGenerator(args.patch_size)
    ]))
...
def self_train(args, pre_snapshot_path, snapshot_path):
    ...
    logging.info(
        "MRI interpolation correction: image rotate/resize order=1; "
        "segmentation label order=0")
