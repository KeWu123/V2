...
def count_labeled_slices(base_dir, labeled_case_count):
    """Count the contiguous slice prefix belonging to labeled train cases."""
    train_path = os.path.join(base_dir, "train.list")
    slices_path = os.path.join(base_dir, "train_slices.list")
    with open(train_path, "r") as handle:
        train_cases = [line.strip().split(".")[0] for line in handle if line.strip()]
    with open(slices_path, "r") as handle:
        slice_names = [line.strip().split(".")[0] for line in handle if line.strip()]
    labeled_cases = train_cases[:int(labeled_case_count)]
    if len(labeled_cases) != int(labeled_case_count):
        raise ValueError(
            "Requested {} labeled cases, but train.list contains {}".format(
                labeled_case_count, len(labeled_cases)
            )
        )
    labeled_set = set(labeled_cases)
    labeled_names = [
        name for name in slice_names
        if name.rsplit("_slice", 1)[0] in labeled_set
    ]
    if slice_names[:len(labeled_names)] != labeled_names:
        raise ValueError(
            "train_slices.list must group the first train.list cases first"
        )
    if not labeled_names:
        raise ValueError("No slices found for labeled cases: {}".format(labeled_cases))
    return len(labeled_names)
