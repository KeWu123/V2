...
class UNet(nn.Module):
    ...
    def forward(self, x, need_fp=False, feature_dropout=0.5):
        ...
        if need_fp:
            ...
            paired_output, _ = self.decoder(paired_feature)
...
parser.add_argument('--exp', type=str,
                    default='MT_PROMISE12_UniMatch_RVTV', help='experiment_name')
...
parser.add_argument('--reference_feature_level', type=int, default=2,
                    help='frozen U-Net encoder level used by the reference bank')
parser.add_argument('--reference_samples_per_slice', type=int, default=32,
                    help='labeled reference features sampled per class and slice')
parser.add_argument('--reference_bank_size', type=int, default=512,
                    help='maximum reference features per zone and class')
parser.add_argument('--reference_topk', type=int, default=5,
                    help='top-k labeled reference similarities')
parser.add_argument('--reference_temperature', type=float, default=0.10,
                    help='reference matching softmax temperature')
parser.add_argument('--reference_target_precision', type=float, default=0.95,
                    help='leave-one-case-out reference calibration precision')
parser.add_argument('--soft_pseudo_weight', type=float, default=0.25,
                    help='soft-target KL weight inside each UniMatch branch')
parser.add_argument('--soft_teacher_threshold', type=float, default=0.70,
                    help='minimum teacher confidence for a soft target')
parser.add_argument('--soft_reference_threshold', type=float, default=0.70,
                    help='minimum reference confidence for a soft target')
parser.add_argument('--reference_mix', type=float, default=0.35,
                    help='reference contribution to the soft pseudo target')
parser.add_argument('--temporal_decay', type=float, default=0.90,
                    help='EMA decay for per-slice temporal area statistics')
parser.add_argument('--temporal_threshold', type=float, default=0.25,
                    help='minimum temporal reliability for hard targets')
parser.add_argument('--volume_threshold', type=float, default=0.25,
                    help='minimum adjacent-slice reliability for hard targets')
...
def self_train(args, pre_snapshot_path, snapshot_path):
    ...
    reference_model = copy.deepcopy(ema_model)
    ...
    if 'net' in checkpoint:
        ...
        reference_model.load_state_dict(checkpoint['net'])
    ...
    # Build a frozen, ground-truth labeled reference source.  It never receives
    # gradients and therefore cannot co-drift with the EMA pseudo labels.
    reference_dataset = BaseDataSets(
        base_dir=args.root_path, split="train", num=None,
        transform=transforms.Compose([RandomGenerator(args.patch_size)]))
    reference_loader = DataLoader(
        Subset(reference_dataset, labeled_idxs),
        batch_size=args.labeled_bs, shuffle=False, num_workers=0,
        pin_memory=True)
    reference_bank = rvtv_pseudo.ReferenceFeatureBank(
        reference_model, reference_dataset.sample_list,
        feature_level=args.reference_feature_level,
        samples_per_slice=args.reference_samples_per_slice,
        max_samples=args.reference_bank_size,
        topk=args.reference_topk,
        temperature=args.reference_temperature)
    logging.info("Building frozen labeled reference bank...")
    python_rng_state = random.getstate()
    numpy_rng_state = np.random.get_state()
    torch_rng_state = torch.get_rng_state()
    cuda_rng_states = torch.cuda.get_rng_state_all()
    try:
        reference_seed = args.seed + 7919
        ...
        reference_bank.build(reference_loader, torch.device('cuda'))
        reference_thresholds = reference_bank.calibrate(
            reference_loader, torch.device('cuda'),
            target_precision=args.reference_target_precision)
    finally:
        random.setstate(python_rng_state)
        np.random.set_state(numpy_rng_state)
        torch.set_rng_state(torch_rng_state)
        torch.cuda.set_rng_state_all(cuda_rng_states)
    temporal_volume_bank = rvtv_pseudo.TemporalVolumeBank(
    ...
    logging.info(
        "RVTV UniMatch: EMA+LCC teacher retained, tau=%.2f, "
    ...
    logging.info(
        "RVTV reference thresholds: background=%.4f foreground=%.4f; "
        "target_precision=%.3f temporal_tau=%.3f volume_tau=%.3f soft_w=%.3f",
        reference_thresholds[0], reference_thresholds[1],
        args.reference_target_precision, args.temporal_threshold,
        args.volume_threshold, args.soft_pseudo_weight)
    logging.info(
        "RVTV labeled calibration: bg_precision=%.4f bg_coverage=%.4f "
        "fg_precision=%.4f fg_coverage=%.4f",
        reference_bank.calibration_metrics[0]['precision'],
        reference_bank.calibration_metrics[0]['coverage'],
        reference_bank.calibration_metrics[1]['precision'],
        reference_bank.calibration_metrics[1]['coverage'])
    calibration_path = os.path.join(snapshot_path, 'rvtv_calibration.csv')
    with open(calibration_path, 'w', newline='', encoding='utf-8') as handle:
        table = csv.writer(handle)
        table.writerow([
            'class', 'reference_threshold', 'loo_precision',
            'loo_coverage', 'accepted_pixels'])
        for class_id, class_name in ((0, 'background'), (1, 'foreground')):
            metrics_i = reference_bank.calibration_metrics[class_id]
            table.writerow([
                class_name, reference_thresholds[class_id],
                metrics_i['precision'], metrics_i['coverage'],
                metrics_i['samples']])
    logging.info("RVTV calibration table: %s", calibration_path)
    ...
    def cutmix_route(route, permutation, boxes):
        return {
            'hard_target': cutmix_tensor(
                route['hard_target'], route['hard_target'][permutation], boxes),
            'hard_valid': cutmix_tensor(
                route['hard_valid'], route['hard_valid'][permutation], boxes),
            'soft_target': cutmix_tensor(
                route['soft_target'], route['soft_target'][permutation], boxes),
            'soft_valid': cutmix_tensor(
                route['soft_valid'], route['soft_valid'][permutation], boxes),
        }
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            unlabeled_cases = [
                str(item) for item in sampled_batch['case'][args.labeled_bs:]]

            # Populate temporal/volume evidence even during the supervised
            # warm-up.  No pseudo-label loss is applied until the fixed gate.
            with torch.no_grad():
                ...
                temporal_scores, volume_scores = (
                    temporal_volume_bank.update_and_score(
                        unlabeled_cases, pseudo_labels))
            ...
            if iter_num < self_warmup_iterations:
                ...
                routed_hard_loss = outputs.new_zeros(())
                routed_soft_loss = outputs.new_zeros(())
                confident_ratio = outputs.new_zeros(())
                confident_fg_ratio = outputs.new_zeros(())
                soft_ratio = outputs.new_zeros(())
                agreement_ratio = outputs.new_zeros(())
            else:
                ...
                with torch.no_grad():
                    reference_probability = reference_bank.probability(
                        unlabeled_volume_batch, unlabeled_cases,
                        output_size=ema_probability.shape[-2:])
                    route = rvtv_pseudo.route_pseudo_labels(
                        ema_probability, pseudo_labels,
                        reference_probability, reference_thresholds,
                        temporal_scores, volume_scores,
                        teacher_threshold=args.confidence_threshold,
                        soft_teacher_threshold=args.soft_teacher_threshold,
                        soft_reference_threshold=args.soft_reference_threshold,
                        temporal_threshold=args.temporal_threshold,
                        volume_threshold=args.volume_threshold,
                        reference_mix=args.reference_mix)
                    route_s1 = cutmix_route(
                        route, permutation1, cutmix_box1)
                    route_s2 = cutmix_route(
                        route, permutation2, cutmix_box2)
                ...
                loss_u_s1, hard_s1, soft_s1 = rvtv_pseudo.routed_pseudo_loss(
                    outputs_s1, route_s1, dice_loss,
                    soft_weight=args.soft_pseudo_weight)
                loss_u_s2, hard_s2, soft_s2 = rvtv_pseudo.routed_pseudo_loss(
                    outputs_s2, route_s2, dice_loss,
                    soft_weight=args.soft_pseudo_weight)
                loss_u_fp, hard_fp, soft_fp = rvtv_pseudo.routed_pseudo_loss(
                    outputs_fp[args.labeled_bs:], route, dice_loss,
                    soft_weight=args.soft_pseudo_weight)
                consistency_loss = (
                    0.25 * loss_u_s1 + 0.25 * loss_u_s2 +
                    0.5 * loss_u_fp)
                routed_hard_loss = (
                    0.25 * hard_s1 + 0.25 * hard_s2 + 0.5 * hard_fp)
                routed_soft_loss = (
                    0.25 * soft_s1 + 0.25 * soft_s2 + 0.5 * soft_fp)
                confident_mask = route['hard_valid']
                ...
                soft_ratio = route['soft_valid'].float().mean()
                agreement_ratio = route['agreement'].float().mean()
            ...
            writer.add_scalar('rvtv/hard_loss', routed_hard_loss, iter_num)
            writer.add_scalar('rvtv/soft_loss', routed_soft_loss, iter_num)
            writer.add_scalar('rvtv/hard_coverage', confident_ratio, iter_num)
            writer.add_scalar('rvtv/soft_coverage', soft_ratio, iter_num)
            writer.add_scalar('rvtv/reference_agreement',
                              agreement_ratio, iter_num)
            writer.add_scalar('rvtv/temporal_score',
                              temporal_scores.mean(), iter_num)
            writer.add_scalar('rvtv/volume_score',
                              volume_scores.mean(), iter_num)
            ...
            if iter_num % 20 == 0:
                ...
                logging.info(
                    ...
                    's1=%.6f s2=%.6f fp=%.6f hard=%.6f soft=%.6f '
                    'hard_cov=%.4f soft_cov=%.4f fg_cov=%.4f agree=%.4f '
                    'temporal=%.4f volume=%.4f',
                    iter_num, max_iterations, loss.item(),
                    supervised_loss.item(), consistency_loss.item(),
                    loss_u_s1.item(), loss_u_s2.item(), loss_u_fp.item(),
                    routed_hard_loss.item(), routed_soft_loss.item(),
                    confident_ratio.item(), soft_ratio.item(),
                    confident_fg_ratio.item(), agreement_ratio.item(),
                    temporal_scores.mean().item(), volume_scores.mean().item())
