# UniMatch V2 (A0) on PROMISE12.
#
# A0: keep the U-Net backbone / EMA teacher / pseudo-label pipeline /
# confidence threshold (0.95) / CutMix / strong augmentation / optimizer /
# validation-test protocol of the UniMatch baseline (train_unimatch.py),
# and replace ONLY the unsupervised branch structure:
#
#   V1: 0.25 * strong_view1 + 0.25 * strong_view2 + 0.50 * feature-perturbation
#   V2: 0.50 * strong_view1 + 0.50 * strong_view2   (Complementary Channel-Wise
#                                                   Dropout on the bottleneck)
#
# The feature-perturbation branch of V1 is removed.
...
parser.add_argument('--exp', type=str,
                    default='UniMatchV2_EMA_momentum_seed1337', help='experiment_name')
...
parser.add_argument('--ema_mode', type=str, default='momentum',
                    choices=('fixed', 'momentum'),
                    help='EMA teacher update: fixed decay (0.99) or V2-official '
                         'momentum ratio min(1-1/(iter+1), cap)')
parser.add_argument('--ema_momentum_max', type=float, default=0.996,
                    help='upper bound of the momentum EMA ratio')
...
def self_train(args, pre_snapshot_path, snapshot_path):
    ...
    logging.info("{} iterations per epoch".format(len(trainloader)))
    logging.info(
        "UniMatch V2 A0 + EMA(%s): EMA+LCC teacher retained, tau=%.2f, "
        "ema_momentum_max=%.3f, comp_drop_dropout_prob=%.2f, "
        "branch_weights=(0.50, 0.50), strong_aug_prob=%.2f, "
        "blur_prob=%.2f, cutmix_prob=%.2f, ema_teacher_mode=%s",
        args.ema_mode, args.confidence_threshold, args.ema_momentum_max,
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            if iter_num < 1000:
                ...
                loss_u_s2, confident_mask = confidence_masked_baseline_loss(
            ...
            # EMA teacher update: fixed decay or V2-official momentum ratio.
            if args.ema_mode == 'momentum':
                ema_alpha = min(1.0 - 1.0 / (iter_num + 1),
                                args.ema_momentum_max)
            else:
                ema_alpha = args.ema_decay
            update_model_ema(model, ema_model, ema_alpha)
