...
from utils import losses, ramps, feature_memory, contrastive_losses, val_2d_25d
...
parser.add_argument('--exp', type=str,
                    default='MT_PROMISE12_UniMatch_25D', help='experiment_name')
...
def pre_train(args, snapshot_path):
    ...
    model = UNet(in_chns=3, class_num=num_classes).cuda()

    def worker_init_fn(worker_id):
        random.seed(args.seed + worker_id)

    db_train = PROMISE12DataSets25D(base_dir=args.root_path, num=None, transform=transforms.Compose([
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            if iter_num > 0 and iter_num % 200 == 0:
                ...
                for _, sampled_batch in enumerate(valloader):
                    metric_i = val_2d_25d.test_single_volume(sampled_batch["image"], sampled_batch["label"], model,
                                                             classes=num_classes)
...
def self_train(args, pre_snapshot_path, snapshot_path):
    ...
    logging.info(
        "UniMatch 2.5D: input=[z-1,z,z+1], EMA+LCC teacher retained, tau=%.2f, "
