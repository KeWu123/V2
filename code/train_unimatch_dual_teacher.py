...
from dataloaders.dataset_25d import RandomGenerator25D
from dataloaders.dataset_dual_teacher import PROMISE12DualTeacherDataSets
...
parser.add_argument('--exp', type=str,
                    default='MT_PROMISE12_UniMatch_DualTeacher', help='experiment_name')
...
parser.add_argument(
    '--aux_25d_checkpoint', type=str,
    default=os.path.abspath(os.path.join(
        os.path.dirname(__file__), '..', 'model',
        'MT_PROMISE12_UniMatch_25D_7_labeled', 'pre_train', 'unet',
        'unet_best_model.pth')),
    help='frozen supervised 2.5D teacher checkpoint')
...
def create_frozen_25d_teacher(checkpoint_path, num_classes):
    """Load the supervised 2.5D model without changing student RNG state."""
    checkpoint_path = os.path.abspath(os.path.expanduser(checkpoint_path))
    if not os.path.isfile(checkpoint_path):
        raise FileNotFoundError(
            "Frozen 2.5D teacher checkpoint not found: {}".format(
                checkpoint_path))

    cpu_rng_state = torch.get_rng_state()
    cuda_rng_states = torch.cuda.get_rng_state_all()
    try:
        teacher = UNet(in_chns=3, class_num=num_classes).cuda()
        try:
            checkpoint = torch.load(
                checkpoint_path, map_location='cpu', weights_only=False)
        except TypeError:
            checkpoint = torch.load(checkpoint_path, map_location='cpu')
        state_dict = checkpoint.get('net', checkpoint) if isinstance(
            checkpoint, dict) else checkpoint
        if not isinstance(state_dict, dict):
            raise RuntimeError(
                "Unsupported 2.5D checkpoint format: {}".format(
                    checkpoint_path))
    ...
    teacher.eval()
    logging.info("Loaded frozen 2.5D teacher from %s", checkpoint_path)
    return teacher
...
def self_train(args, pre_snapshot_path, snapshot_path):
    ...
    auxiliary_teacher = create_frozen_25d_teacher(
        args.aux_25d_checkpoint, num_classes)

    def worker_init_fn(worker_id):
        random.seed(args.seed + worker_id)

    db_train = PROMISE12DualTeacherDataSets(base_dir=args.root_path, num=None, transform=transforms.Compose([
        RandomGenerator25D(args.patch_size)
    ]))
    db_val = BaseDataSets(base_dir=args.root_path, split="val")
    ...
    logging.info(
        "UniMatch dual teacher: original 2D EMA targets retained; frozen 2.5D "
        "teacher is agreement-only, tau=%.2f, "
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            context_batch = sampled_batch['context_image'].cuda()
            label_batch = sampled_batch['label'].cuda()
            unlabeled_volume_batch = volume_batch[args.labeled_bs:]
            unlabeled_context_batch = context_batch[args.labeled_bs:]
            ...
            if iter_num < self_warmup_iterations:
                ...
                teacher_agreement = outputs.new_zeros(())
                teacher_fg_agreement = outputs.new_zeros(())
                ema_confident_ratio = outputs.new_zeros(())
                ema_confident_fg_ratio = outputs.new_zeros(())
            else:
                ...
                with torch.no_grad():
                    ...
                    pseudo_foreground = pseudo_labels == 1
                    ema_confident_mask = (
                        pseudo_confidence >= args.confidence_threshold)
                    ema_confident_ratio = ema_confident_mask.float().mean()
                    ema_confident_fg_ratio = (
                        (ema_confident_mask & pseudo_foreground).float().sum() /
                        pseudo_foreground.float().sum().clamp_min(1.0))

                    auxiliary_output = auxiliary_teacher(
                        unlabeled_context_batch)
                    if isinstance(auxiliary_output, tuple):
                        auxiliary_output = auxiliary_output[0]
                    auxiliary_labels = get_masks(
                        auxiliary_output, nms=1).long()

                    # Preserve the original EMA pseudo target. The auxiliary
                    # model can only reject a pixel when the two teachers
                    # disagree; it never replaces or adds a target.
                    agreement_mask = pseudo_labels.eq(auxiliary_labels)
                    pseudo_confidence = pseudo_confidence * agreement_mask.float()
                    teacher_agreement = agreement_mask.float().mean()
                    teacher_fg_agreement = (
                        (agreement_mask & pseudo_foreground).float().sum() /
                        pseudo_foreground.float().sum().clamp_min(1.0))
            ...
            writer.add_scalar('dual_teacher/agreement',
                              teacher_agreement, iter_num)
            writer.add_scalar('dual_teacher/foreground_agreement',
                              teacher_fg_agreement, iter_num)
            writer.add_scalar('dual_teacher/ema_coverage_before_filter',
                              ema_confident_ratio, iter_num)
            writer.add_scalar('dual_teacher/ema_foreground_coverage_before_filter',
                              ema_confident_fg_ratio, iter_num)
            ...
            if iter_num % 20 == 0:
                ...
                logging.info(
                    ...
                    'ema_coverage=%.4f ema_fg_coverage=%.4f '
                    'agreement=%.4f fg_agreement=%.4f',
                    ...
                    ema_confident_ratio.item(), ema_confident_fg_ratio.item(),
                    teacher_agreement.item(), teacher_fg_agreement.item())
