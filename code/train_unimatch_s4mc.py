...
parser.add_argument('--exp', type=str,
                    default='MT_PROMISE12_UniMatch_S4MC', help='experiment_name')
...
parser.add_argument('--s4mc_start_keep_percent', type=float, default=60.0,
                    help='initial percentage of foreground pseudo pixels kept by S4MC')
parser.add_argument('--s4mc_end_keep_percent', type=float, default=100.0,
                    help='final percentage of foreground pseudo pixels kept by S4MC')
parser.add_argument('--s4mc_neighborhood', type=int, default=4, choices=[4, 8],
                    help='number of spatial neighbours used by S4MC')
parser.add_argument('--s4mc_num_neighbors', type=int, default=1,
                    help='number of strongest neighbours used to refine probability')
...
def reliability_masked_baseline_loss(logits, targets, valid, dice_loss):
    """Apply the unchanged UniMatch CE+Dice pseudo loss at reliable pixels."""
...
def s4mc_neighbor_tensors(probability, neighborhood):
    """Return four/eight neighbours as in the official S4MC implementation."""
    if neighborhood not in (4, 8):
        raise ValueError("S4MC neighborhood must be 4 or 8")
    padded = F.pad(probability, (1, 1, 1, 1))
...
def s4mc_keep_percent(iter_num, max_iterations):
    """Official S4MC linear schedule, restricted to the active SSL phase."""
    ssl_start = 1000
    progress = (iter_num - ssl_start) / max(1, max_iterations - ssl_start)
    progress = min(1.0, max(0.0, progress))
    return (
        args.s4mc_start_keep_percent +
        progress * (args.s4mc_end_keep_percent - args.s4mc_start_keep_percent)
    )


def s4mc_reliable_mask(probability, pseudo_labels, iter_num, max_iterations):
    """Build the UniMatch trusted core plus S4MC-supported foreground.

    The margin and strongest-neighbour refinement follow the official S4MC
    loss implementation. PROMISE12 adaptation is deliberately narrow:
    percentile statistics use only LCC foreground pixels, and a neutral
    class expectation replaces Pascal/Cityscapes-specific class statistics.
    """
    core = probability.gather(
        1, pseudo_labels.unsqueeze(1)).squeeze(1) >= args.confidence_threshold
    foreground = pseudo_labels == 1
    keep_percent = s4mc_keep_percent(iter_num, max_iterations)

    top2 = torch.topk(probability, 2, dim=1).values
    raw_margin = top2[:, 0] - top2[:, 1]
    if not foreground.any():
        threshold = raw_margin.new_tensor(1.0)
        added = torch.zeros_like(foreground)
        return core, core, added, threshold, keep_percent, raw_margin, raw_margin

    quantile = max(0.0, min(1.0, (100.0 - keep_percent) / 100.0))
    threshold = torch.quantile(raw_margin[foreground].float(), quantile).to(
        raw_margin.dtype)

    neighbors = torch.stack(
        s4mc_neighbor_tensors(probability, args.s4mc_neighborhood), dim=0)
    neighbor_count = min(max(1, args.s4mc_num_neighbors), neighbors.shape[0])
    strongest = torch.topk(neighbors, k=neighbor_count, dim=0).values

    refined = probability.clone()
    beta = refined.new_tensor(-0.5).exp()
    expectation = torch.ones(
        (1, probability.shape[1], 1, 1),
        dtype=probability.dtype, device=probability.device)
    for neighbor in strongest:
        refined = (
            refined + beta * neighbor -
            beta * torch.maximum(refined * neighbor, refined * expectation)
        )

    refined_top2, refined_label = torch.topk(refined, 2, dim=1)
    refined_margin = refined_top2[:, 0] - refined_top2[:, 1]
    context_foreground = (
        foreground & (refined_label[:, 0] == 1) &
        (refined_margin > threshold)
    )
    added = context_foreground & ~core
    reliable = core | context_foreground
    return reliable, core, added, threshold, keep_percent, raw_margin, refined_margin
...
def self_train(args, pre_snapshot_path, snapshot_path):
    ...
    logging.info(
        "UniMatch+S4MC: EMA+LCC teacher retained, tau=%.2f, "
        "feature_dropout=%.2f, branch_weights=(0.25, 0.25, 0.50), "
        "strong_aug_prob=%.2f, blur_prob=%.2f, cutmix_prob=%.2f, "
        "foreground_keep=%.1f->%.1f, neighborhood=%d, neighbors=%d",
        args.confidence_threshold, args.feature_dropout,
        args.strong_aug_prob, args.blur_prob, args.cutmix_prob,
        args.s4mc_start_keep_percent, args.s4mc_end_keep_percent,
        args.s4mc_neighborhood, args.s4mc_num_neighbors)
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            if iter_num < self_warmup_iterations:
                ...
                core_ratio = outputs.new_zeros(())
                s4mc_added_ratio = outputs.new_zeros(())
                s4mc_added_fg_ratio = outputs.new_zeros(())
                s4mc_threshold = outputs.new_zeros(())
                s4mc_keep = 0.0
            else:
                ...
                with torch.no_grad():
                    ...
                    (
                        reliable_mask, core_mask, s4mc_added,
                        s4mc_threshold, s4mc_keep, _, _
                    ) = s4mc_reliable_mask(
                    ...
                    reliable_s1 = cutmix_tensor(
                        reliable_mask,
                        reliable_mask[permutation1], cutmix_box1)
                    reliable_s2 = cutmix_tensor(
                        reliable_mask,
                        reliable_mask[permutation2], cutmix_box2)
                ...
                loss_u_s1, _ = reliability_masked_baseline_loss(
                    outputs_s1, pseudo_labels_s1, reliable_s1, dice_loss)
                loss_u_s2, _ = reliability_masked_baseline_loss(
                    outputs_s2, pseudo_labels_s2, reliable_s2, dice_loss)
                loss_u_fp, confident_mask = reliability_masked_baseline_loss(
                    outputs_fp[args.labeled_bs:], pseudo_labels,
                    reliable_mask, dice_loss)
                consistency_loss = (
                    0.25 * loss_u_s1 + 0.25 * loss_u_s2 +
                    0.5 * loss_u_fp)
                confident_ratio = confident_mask.float().mean()
                core_ratio = core_mask.float().mean()
                s4mc_added_ratio = s4mc_added.float().mean()
                foreground = pseudo_labels == 1
                confident_fg_ratio = (
                    (confident_mask & foreground).float().sum() /
                    foreground.float().sum().clamp_min(1.0))
                s4mc_added_fg_ratio = (
                    s4mc_added.float().sum() /
                    foreground.float().sum().clamp_min(1.0))
            ...
            writer.add_scalar('s4mc/core_ratio', core_ratio, iter_num)
            writer.add_scalar('s4mc/added_ratio', s4mc_added_ratio, iter_num)
            writer.add_scalar('s4mc/added_foreground_ratio',
                              s4mc_added_fg_ratio, iter_num)
            writer.add_scalar('s4mc/foreground_margin_threshold',
                              s4mc_threshold, iter_num)
            writer.add_scalar('s4mc/keep_percent', s4mc_keep, iter_num)
            ...
            if iter_num % 20 == 0:
                ...
                logging.info(
                    ...
                    'fg_coverage=%.4f core=%.4f s4mc_added=%.4f '
                    's4mc_fg_added=%.4f margin_tau=%.4f keep=%.1f',
                    ...
                    core_ratio.item(), s4mc_added_ratio.item(),
                    s4mc_added_fg_ratio.item(), s4mc_threshold.item(),
                    s4mc_keep)
