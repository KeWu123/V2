...
# RTX 5090 launcher for PROMISE12 Baseline + UniMatch + RVTV routing.
...
GPU="${GPU:-0}"
SEED="${SEED:-1337}"
EXP_NAME="${EXP_NAME:-MT_PROMISE12_UniMatch_RVTV}"
DETACH="${DETACH:-0}"
...
if [[ "${DETACH}" == "1" && "${_DESKTOP_UNIMATCH_DETACHED:-0}" != "1" ]]; then
    SERVER_LOG="${SERVER_LOG_DIR}/train_$(date +%Y%m%d_%H%M%S).log"
    nohup env _DESKTOP_UNIMATCH_DETACHED=1 DETACH=0 \
...
[[ -f "${CODE_DIR}/train_unimatch_rvtv.py" ]] || {
    echo "Missing training entry: ${CODE_DIR}/train_unimatch_rvtv.py" >&2
    exit 2
}
[[ -f "${CONVERTER}" ]] || {
...
if command -v flock >/dev/null 2>&1; then
    exec 9>"${SERVER_LOG_DIR}/.unimatch_rvtv_training.lock"
    flock -n 9 || {
...
syntax_targets = [
    code_dir / "train_unimatch_rvtv.py",
    code_dir / "rvtv_pseudo.py",
...
echo "RVTV: frozen labeled reference + calibrated hard/soft/ignore + temporal/volume reliability"
printf 'Launching:'
printf ' %q' env CUDA_VISIBLE_DEVICES="${GPU}" "${PYTHON_LAUNCHER[@]}" \
    train_unimatch_rvtv.py --root_path "${DATA_ROOT}" --exp "${EXP_NAME}" --seed "${SEED}"
printf '\n'

RUN_LOG="${SERVER_LOG_DIR}/train_console_$(date +%Y%m%d_%H%M%S).log"
CUDA_VISIBLE_DEVICES="${GPU}" "${PYTHON_LAUNCHER[@]}" \
    train_unimatch_rvtv.py --root_path "${DATA_ROOT}" \
...
echo "Run test_and_quantify_unimatch_rvtv_5090.sh to evaluate both checkpoints."
