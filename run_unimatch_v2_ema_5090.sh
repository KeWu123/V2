...
# RTX 5090 launcher for the PROMISE12 UniMatch V2 EMA experiment.
#
# Inherits the UniMatch V2 A0 protocol (35/5/10, 7 labeled cases, seed 1337,
# pretrain 10000 / self-train 30000, batch 24/12, SGD lr 0.01, tau=0.95,
# CutMix, strong MRI augmentation, comp_drop dual-view) and replaces ONLY the
# EMA teacher update with the V2-official momentum ratio
# (see code/train_unimatch_v2_ema.py).
...
GPU="${GPU:-0}"
SEED="${SEED:-1337}"
EXP_NAME="${EXP_NAME:-UniMatchV2_EMA_momentum_seed1337}"
PRE_ITERATIONS="${PRE_ITERATIONS:-10000}"
MAX_ITERATIONS="${MAX_ITERATIONS:-30000}"
EMA_MODE="${EMA_MODE:-momentum}"
EMA_MOMENTUM_MAX="${EMA_MOMENTUM_MAX:-0.996}"
DETACH="${DETACH:-0}"
...
if [[ "${DETACH}" == "1" && "${_DESKTOP_UNIMATCH_V2_EMA_DETACHED:-0}" != "1" ]]; then
    SERVER_LOG="${SERVER_LOG_DIR}/train_$(date +%Y%m%d_%H%M%S).log"
    nohup env _DESKTOP_UNIMATCH_V2_EMA_DETACHED=1 DETACH=0 \
...
[[ -f "${CODE_DIR}/train_unimatch_v2_ema.py" ]] || {
    echo "Missing training entry: ${CODE_DIR}/train_unimatch_v2_ema.py" >&2
    exit 2
}
[[ -f "${CONVERTER}" ]] || {
...
if command -v flock >/dev/null 2>&1; then
    exec 9>"${SERVER_LOG_DIR}/.unimatch_v2_ema_training.lock"
    flock -n 9 || {
        echo "Another UniMatch V2 EMA training process is already running." >&2
        exit 3
    }
fi
...
syntax_targets = [
    code_dir / "train_unimatch_v2_ema.py",
...
echo "EMA defaults: mode=${EMA_MODE}, momentum_max=${EMA_MOMENTUM_MAX}"
printf 'Launching:'
printf ' %q' env CUDA_VISIBLE_DEVICES="${GPU}" "${PYTHON_LAUNCHER[@]}" \
    train_unimatch_v2_ema.py --root_path "${DATA_ROOT}" --exp "${EXP_NAME}" --seed "${SEED}" \
    --pre_iterations "${PRE_ITERATIONS}" --max_iterations "${MAX_ITERATIONS}" \
    --ema_mode "${EMA_MODE}" --ema_momentum_max "${EMA_MOMENTUM_MAX}" "$@"
printf '\n'

RUN_LOG="${SERVER_LOG_DIR}/train_console_$(date +%Y%m%d_%H%M%S).log"
CUDA_VISIBLE_DEVICES="${GPU}" "${PYTHON_LAUNCHER[@]}" \
    train_unimatch_v2_ema.py --root_path "${DATA_ROOT}" \
