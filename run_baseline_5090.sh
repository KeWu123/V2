...
# RTX 5090 launcher for THIS folder's original baseline method.
# Only dataset/output bookkeeping and the random seed are supplied to Python.
# Training settings use the defaults in train_baseline.py (pretrain 1000,
# self-train 5000, fixed warm-up 1000); the baseline method is unchanged.
...
GPU="${GPU:-0}"
SEED="${SEED:-1337}"
EXP_NAME="${EXP_NAME:-MT_PROMISE12_baseline_191slices}"
DETACH="${DETACH:-0}"
...
if [[ "${DETACH}" == "1" && "${_DESKTOP_BASELINE_DETACHED:-0}" != "1" ]]; then
    SERVER_LOG="${SERVER_LOG_DIR}/train_$(date +%Y%m%d_%H%M%S).log"
    nohup env _DESKTOP_BASELINE_DETACHED=1 DETACH=0 \
...
[[ -f "${CODE_DIR}/train_baseline.py" ]] || {
    echo "Missing training entry: ${CODE_DIR}/train_baseline.py" >&2
    exit 2
}
[[ -f "${CONVERTER}" ]] || {
...
if command -v flock >/dev/null 2>&1; then
    exec 9>"${SERVER_LOG_DIR}/.training.lock"
    flock -n 9 || {
...
echo "Configured defaults: PROMISE12 35/5/10, labelnum=7 (dynamic slice count), pre=1000, self=5000, warmup=1000, batch=24, labeled_bs=12, lr=0.01"
printf 'Launching:'
printf ' %q' env CUDA_VISIBLE_DEVICES="${GPU}" "${PYTHON_LAUNCHER[@]}" \
    train_baseline.py --root_path "${DATA_ROOT}" --exp "${EXP_NAME}" --seed "${SEED}"
printf '\n'

RUN_LOG="${SERVER_LOG_DIR}/train_console_$(date +%Y%m%d_%H%M%S).log"
CUDA_VISIBLE_DEVICES="${GPU}" "${PYTHON_LAUNCHER[@]}" \
    train_baseline.py --root_path "${DATA_ROOT}" \
...
echo "Run test_and_quantify_baseline_5090.sh to evaluate both checkpoints."
