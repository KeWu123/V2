...
# Test both checkpoints from the UniMatch dual-teacher experiment, then create
...
[[ -f "${CODE_DIR}/test_unimatch_dual_teacher.py" ]] || {
    echo "Missing test entry: ${CODE_DIR}/test_unimatch_dual_teacher.py" >&2
    exit 2
}
...
run_stage() {
    ...
    CUDA_VISIBLE_DEVICES="${GPU}" "${PYTHON_LAUNCHER[@]}" \
        test_unimatch_dual_teacher.py "${test_args[@]}"
}
