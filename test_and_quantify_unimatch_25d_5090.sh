...
# Test both checkpoints from the original UniMatch + 2.5D experiment, then create
...
[[ -f "${CODE_DIR}/test_unimatch_25d.py" ]] || {
    echo "Missing test entry: ${CODE_DIR}/test_unimatch_25d.py" >&2
    exit 2
}
...
run_stage() {
    ...
    CUDA_VISIBLE_DEVICES="${GPU}" "${PYTHON_LAUNCHER[@]}" \
        test_unimatch_25d.py "${test_args[@]}"
}
