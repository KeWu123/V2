...
# RTX 5090 launcher for the PROMISE12 Baseline + UniMatch experiment.
...
GPU="${GPU:-0}"
SEED="${SEED:-1337}"
EXP_NAME="${EXP_NAME:-MT_PROMISE12_UniMatch_191slices}"
DETACH="${DETACH:-0}"
...
[[ -f "${CODE_DIR}/train_unimatch.py" ]] || {
    echo "Missing training entry: ${CODE_DIR}/train_unimatch.py" >&2
    exit 2
}
[[ -f "${CONVERTER}" ]] || {
...
echo "Run test_and_quantify_unimatch_5090.sh to evaluate both checkpoints."
