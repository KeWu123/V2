...
# RTX 5090 launcher for the PROMISE12 UniMatch V2 A0 experiment.
#
# A0 keeps the UniMatch baseline protocol (35/5/10, 7 labeled cases, seed 1337,
# pretrain 10000 / self-train 30000, batch 24/12, SGD lr 0.01, EMA 0.99,
# tau=0.95, CutMix, strong MRI augmentation) and replaces ONLY the
# unsupervised branch structure with two strong views + Complementary
# Channel-Wise Dropout (0.50 / 0.50). The feature-perturbation branch of V1 is
# removed.
...
GPU="${GPU:-0}"
SEED="${SEED:-1337}"
EXP_NAME="${EXP_NAME:-UniMatchV2_A0_label7_seed1337}"
...
if [[ "${DETACH}" == "1" && "${_DESKTOP_UNIMATCH_V2_DETACHED:-0}" != "1" ]]; then
    SERVER_LOG="${SERVER_LOG_DIR}/train_$(date +%Y%m%d_%H%M%S).log"
    nohup env _DESKTOP_UNIMATCH_V2_DETACHED=1 DETACH=0 \
...
[[ -f "${CODE_DIR}/train_unimatch_v2.py" ]] || {
    echo "Missing training entry: ${CODE_DIR}/train_unimatch_v2.py" >&2
    exit 2
}
[[ -f "${CONVERTER}" ]] || {
...
if command -v flock >/dev/null 2>&1; then
    exec 9>"${SERVER_LOG_DIR}/.unimatch_v2_training.lock"
    flock -n 9 || {
        echo "Another UniMatch V2 training process is already running." >&2
        exit 3
    }
fi
...
echo "UniMatch V2 A0 defaults: tau=0.95, comp_drop_dropout_prob=0.5, branch_weights=(0.50, 0.50), strong/blur/CutMix=0.8/0.5/0.5"
printf 'Launching:'
printf ' %q' env CUDA_VISIBLE_DEVICES="${GPU}" "${PYTHON_LAUNCHER[@]}" \
    train_unimatch_v2.py --root_path "${DATA_ROOT}" --exp "${EXP_NAME}" --seed "${SEED}" \
...
CUDA_VISIBLE_DEVICES="${GPU}" "${PYTHON_LAUNCHER[@]}" \
    train_unimatch_v2.py --root_path "${DATA_ROOT}" \
...
echo "Run test_and_quantify_unimatch_v2_5090.sh to evaluate both checkpoints."
