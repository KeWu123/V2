...
# PROMISE12 original 2D UniMatch + frozen 2.5D agreement teacher.
...
GPU="${GPU:-0}"
SEED="${SEED:-1337}"
EXP_NAME="${EXP_NAME:-MT_PROMISE12_UniMatch_DualTeacher}"
DETACH="${DETACH:-0}"
EXPERIMENT_DIR="${BASELINE_ROOT}/model/${EXP_NAME}_7_labeled"
AUX_25D_CHECKPOINT="${AUX_25D_CHECKPOINT:-${BASELINE_ROOT}/model/MT_PROMISE12_UniMatch_25D_7_labeled/pre_train/unet/unet_best_model.pth}"
LOG_DIR="${BASELINE_ROOT}/server_logs"
SCRIPT_PATH="${BASELINE_ROOT}/$(basename -- "${BASH_SOURCE[0]}")"

mkdir -p "${LOG_DIR}"

if [[ "${DETACH}" == "1" && "${_UNIMATCH_DUAL_DETACHED:-0}" != "1" ]]; then
    LOG_PATH="${LOG_DIR}/train_dual_teacher_$(date +%Y%m%d_%H%M%S).log"
    nohup env _UNIMATCH_DUAL_DETACHED=1 DETACH=0 \
...
[[ -f "${CODE_DIR}/train_unimatch_dual_teacher.py" ]] || {
    echo "Missing ${CODE_DIR}/train_unimatch_dual_teacher.py" >&2
    exit 2
}
h5_ready() {
    local first_slice
...
[[ -f "${AUX_25D_CHECKPOINT}" ]] || {
    echo "Missing frozen 2.5D pretrain checkpoint: ${AUX_25D_CHECKPOINT}" >&2
    echo "Set AUX_25D_CHECKPOINT to the downloaded 2.5D pretrain unet_best_model.pth if needed." >&2
    exit 2
}

"${PYTHON[@]}" -m py_compile \
    "${CODE_DIR}/train_unimatch_dual_teacher.py" \
    "${CODE_DIR}/dataloaders/dataset_25d.py" \
    "${CODE_DIR}/dataloaders/dataset_dual_teacher.py"
"${PYTHON[@]}" - "${AUX_25D_CHECKPOINT}" <<'PY'
import sys
import torch
...
if not isinstance(state, dict):
    raise SystemExit(f"Unsupported auxiliary checkpoint: {path}")
weights = [
    value for key, value in state.items()
    if key.endswith("encoder.in_conv.conv_conv.0.weight")
]
if len(weights) != 1 or tuple(weights[0].shape[:2]) != (16, 3):
    shape = tuple(weights[0].shape) if weights else None
    raise SystemExit(f"Expected a 3-channel 2.5D U-Net checkpoint, got {shape}: {path}")
print(f"Frozen 2.5D checkpoint passed: {path}")
PY
...
echo "Aux:    ${AUX_25D_CHECKPOINT}"
echo "Config: original 2D UniMatch + frozen 2.5D agreement filter, pre=1000 self=5000 warmup=1000 labelnum=7 batch=24/12"

cd "${CODE_DIR}"
RUN_LOG="${LOG_DIR}/train_dual_teacher_console_$(date +%Y%m%d_%H%M%S).log"
CUDA_VISIBLE_DEVICES="${GPU}" "${PYTHON[@]}" \
    train_unimatch_dual_teacher.py \
    --root_path "${DATA_ROOT}" \
    --exp "${EXP_NAME}" \
    --seed "${SEED}" \
    --aux_25d_checkpoint "${AUX_25D_CHECKPOINT}" 2>&1 | tee "${RUN_LOG}"

echo "Training completed"
echo "Console log: ${RUN_LOG}"
echo "Run: bash test_and_quantify_unimatch_dual_teacher_5090.sh"
