...
# Stable v2 stage-2 refinement from a fixed UniMatch checkpoint.
# The UniMatch pre-training/self-training stages are not rerun.
...
OUTPUT_DIR="${MODEL_ROOT}/${EXP_NAME}_${LABELNUM}_labeled/temporal_bank/unet"
...
if [[ "${DETACH}" == "1" && "${_TEMPORAL_BANK_DETACHED:-0}" != "1" ]]; then
    SERVER_LOG="${SERVER_LOG_DIR}/temporal_bank_$(date +%Y%m%d_%H%M%S).log"
    nohup env _TEMPORAL_BANK_DETACHED=1 DETACH=0 \
...
if [[ -f "${OUTPUT_DIR}/last_checkpoint.pth" && "${ALLOW_EXISTING:-0}" != "1" ]]; then
    echo "Existing refinement found: ${OUTPUT_DIR}" >&2
    echo "Use a new EXP_NAME, or set ALLOW_EXISTING=1 to start a new run in it." >&2
    exit 3
fi
...
echo "Pseudo labels:     coverage-aware bank + 60% volume curriculum + MC8"

cd "${CODE_DIR}"
CUDA_VISIBLE_DEVICES="${GPU}" "${PYTHON_LAUNCHER[@]}" \
    train_unimatch_temporal_bank_v2.py \
        --root_path "${DATA_ROOT}" \
...
echo "Test: EXP_NAME=${EXP_NAME} bash test_and_quantify_unimatch_temporal_bank_v2_5090.sh"
